...
class BertProcessor(object):
    ...
    # Gets a collection of `InputExample`s for the train set  # :param data_dir: # :return:
    def get_train_examples(self, data_dir):
        raise NotImplementedError()

    # Gets a collection of `InputExample`s for the dev set # :param data_dir: # :return:
    def get_dev_examples(self, data_dir):
        raise NotImplementedError()
    
    # Gets a collection of `InputExample`s for the test set # :param data_dir: # :return:
    def get_test_examples(self, data_dir):
        raise NotImplementedError()
    
    # Gets a list of possible labels in the dataset  :return:
    def get_labels(self):
        raise NotImplementedError()

    # Reads a Tab Separated Values (TSV) file # :param input_file: # :param quotechar: # :return:
    @classmethod
...
def convert_examples_to_features(examples, max_seq_length, tokenizer, print_examples=False):
    # Loads a data file into a list of InputBatch objects #:param examples: #:param max_seq_length: #:param tokenizer: #:param print_examples: #:return: a list of InputBatch objects
    features = []
...
def convert_examples_to_hierarchical_features(examples, max_seq_length, tokenizer, print_examples=False):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if example.text_b:
            ...
            # Modifies `tokens_a` and `tokens_b` in place so that the total length is less than the specified length # Account for [CLS], [SEP], [SEP]
            _truncate_seq_pair(tokens_a, tokens_b, max_seq_length - 3)
        else:
...
# Truncates a sequence pair in place to the maximum length # :param tokens_a: # :param tokens_b: # :param max_length: # :return:
# This is a simple heuristic which will always truncate the longer sequence one token at a time. This makes more sense than truncating an equal percent
# of tokens from each, since if one sequence is very short then each token that's truncated likely contains more information than a longer sequence.
